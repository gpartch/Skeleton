...
class Skeleton{
   public:
      ...
      // reset limb angles for all limbs

      // draw bone - RGB parameters + from origin to a point XYZ using vbo
      // draw leg -
      // draw arm - 
      // draw limb labels -

   private:
      vbo_t readBoneFile(string);
      void newBone(vbo_t);
      ...
      string bones_adr = "./bones"; 
      // file names for bones
      string bones_f[NUM_BONES] = 
      {
      ...
      // length of bones TODO: *LENGTHS ARE ESTIMATED, FIX LATER*
      unsigned int bones_l[NUM_BONES]
      {
